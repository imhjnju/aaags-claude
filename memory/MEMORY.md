...
- **First-loss gate status**: `Gate_P2_ConicOpacity`, `Gate_P3_RgbColors`, and `Gate_P4_Radii` are open and PASSing for eval_3D opacity/RGB/radii parity. CUDA's eval_3D golden stores opacity as flat `((float*)conic_opacity)[gid]`; inactive CUDA RGB rows are not semantically stable. P4 requires exact active-set match, masks sentinel-scale `tan(±pi/2-epsilon)` radii, and bounds finite ceil-boundary off-by-one deltas to `max(32, ceil(0.01% of finite radii))`.
- **Next action**: Continue the CUDA intermediate-state ladder with Gate_P5 sorted IDs, preserving the P2-P4 semantic masks and diagnostics.
