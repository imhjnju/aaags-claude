...
// A single descriptor set is allocated once and updated in-place before
// each dispatch (allocate_empty_descriptor_set + update_ssbo/update_ubo),
// matching the pattern used by PreprocessPass et al.
//
    ...
    : ctx_(ctx), beta1_(beta1), beta2_(beta2), eps_(eps)
{
    ...
    // 3. Allocate one descriptor set; update in-place each step_group() call.
    descriptor_set_ = pipeline_->allocate_empty_descriptor_set();

    // 4. Allocate the 32-byte UBO (host-visible coherent).
    ubo_buf_ = std::make_unique<VulkanBuffer>(
        ctx_,
        sizeof(AdamStepUBO),
                             ...
                             uint32_t step)
{
    ...
    // 2. Update all bindings on the pre-allocated descriptor set.
    VkDescriptorSet ds = descriptor_set_;
    ...
    pipeline_->update_ubo (ds, 4, ubo_buf_->handle(), sizeof(AdamStepUBO));
