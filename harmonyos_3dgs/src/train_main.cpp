...
// Allocate and fill RawGaussianParams from loaded (activated) GaussianData.
// Caller owns the returned arrays (heap-allocated).
static RawGaussianParams initRawFromGaussianData(const GaussianData& g) {
    ...
    // Scales: log(activated_scale)
    for (int i = 0; i < N * 3; i++)
        raw.raw_scales[i] = std::log(std::max(1e-12f, g.scales[i]));

    // Rotations: copy as-is (already normalized; raw holds unnormalized copy)
    std::memcpy(raw.raw_rotations, g.rotations, N * 4 * sizeof(float));

    // SH coefficients: identity
...
static bool savePly(const char* path, const RawGaussianParams& raw) {
    ...
    // Write binary vertex data: raw (pre-activation) values
    // SH: reorder from basis-interleaved back to channel-first for PLY
    ...
    for (int i = 0; i < N; i++) {
        ...
        // SH rest: basis-interleaved [k*3+c] -> channel-first
        for (int c = 0; c < 3; c++)
...
// ---------------------------------------------------------------------------
// Build camera matrices (reused from main.cpp patterns)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Multi-camera JSON loading (same format as renderer main.cpp)
// ---------------------------------------------------------------------------
struct CameraWithImage {
    Camera cam;
    std::string img_name;  // image filename from JSON (optional)
};
...
static std::vector<CameraWithImage> loadCamerasJson(const char* json_path) {
    ...
    while (true) {
        ...
        if (fx <= 0 || fy <= 0) continue; // skip non-camera objects
        ...
        // W2C = R_c2w^T, translation = -R_c2w^T * cam_center
        float t[3] = {
...
struct TrainArgs {
    ...
    const char* gt_path = nullptr;       // single GT image (PPM)
    const char* cameras_json = nullptr;  // multi-view cameras JSON
    const char* gt_dir = nullptr;        // directory of GT images for multi-view
    ...
    int sh_degree = -1;  // -1 = use from PLY
    float lr_scale = 1.0f;
    int save_every = 0;   // checkpoint interval (0 = only at end)
    int log_every = 100;
};
...
static TrainArgs parseArgs(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (match("--lr_scale"))  args.lr_scale = (float)atof(argv[++i]);
...
static void printUsage(const char* prog) {
    ...
    printf("  --iterations <N>      Training iterations (default: 1000)\n");
    ...
    printf("  --lr_scale <val>      Learning rate multiplier (default: 1.0)\n");
...
int main(int argc, char** argv) {
    ...
    // Initialize raw params from loaded PLY
    ...
    tcfg.lr_position_init  *= args.lr_scale;
    tcfg.lr_position_final *= args.lr_scale;
    tcfg.lr_feature        *= args.lr_scale;
    tcfg.lr_opacity        *= args.lr_scale;
    tcfg.lr_scaling        *= args.lr_scale;
    tcfg.lr_rotation       *= args.lr_scale;
    tcfg.use_adam = true;

    // Build training views (camera + GT image pairs)
    ...
    if (args.cameras_json) {
        ...
        for (auto& entry : cam_entries) {
            ...
            // Try to load GT image from gt_dir
                ...
                for (auto ext : exts) {
                    ...
                    if (!img.empty()) {
                        ...
                        if (iw != tv.cam.width || ih != tv.cam.height) {
                            ...
                            tv.gt_image = std::move(resized);
                        } else {
                            tv.gt_image = std::move(img);
                        }
                        loaded = true;
                        break;
                    }
        ...
        if (args.gt_path) {
            ...
            // Use GT dimensions if they differ
            ...
            // No GT provided: render current model as GT (self-supervised test mode)
            printf("No GT image provided. Rendering current model as ground truth.\n");
            Renderer gt_renderer(
            ...
            gt_renderer.render(model.data, tv.cam, cfg, tv.gt_image.data());
        }

        views.push_back(std::move(tv));
    }

    model.free();  // no longer need the loaded data
    ...
    // Allocate trainer
    size_t arena_size = 512ULL * 1024 * 1024;
    if (raw.count > 50000) arena_size = 2ULL * 1024 * 1024 * 1024;
    Trainer trainer(arena_size);

    // Random view selection for multi-view
    std::mt19937 rng(42);
    ...
    printf("  LR scale:   %.4f\n", args.lr_scale);
    printf("  LR pos:     %.6f -> %.6f\n", tcfg.lr_position_init, tcfg.lr_position_final);
    printf("  LR feature: %.4f\n", tcfg.lr_feature);
    printf("  LR opacity: %.4f\n", tcfg.lr_opacity);
    printf("  LR scale:   %.4f\n", tcfg.lr_scaling);
    printf("  LR rotation:%.4f\n", tcfg.lr_rotation);
    printf("\n");
    ...
    for (int iter = 0; iter < args.iterations; iter++) {
        // Select view: random for multi-view, 0 for single
        int view_idx = (views.size() > 1) ? view_dist(rng) : 0;
        auto& tv = views[view_idx];

        auto result = trainer.step(raw, tv.cam, tv.gt_image.data(), cfg, tcfg, iter);
        ...
        if (iter == 0 || (iter + 1) % args.log_every == 0 || iter == args.iterations - 1) {
            ...
            if (views.size() > 1)
                printf("iter %5d/%d  loss=%.6f  view=%d  (%.1f it/s)\n",
                       iter + 1, args.iterations, result.loss, view_idx, iter_per_sec);
            else
                printf("iter %5d/%d  loss=%.6f  (%.1f it/s)\n",
                       iter + 1, args.iterations, result.loss, iter_per_sec);
        }
    ...
    if (!savePly(args.output_path, raw)) {
