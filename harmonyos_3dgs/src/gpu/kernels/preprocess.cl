...
)
{
    ...
    if (eval_3D) {
        ...
        // SH
        float rgb[3];
        computeColorFromSH(sh_degree, max_coeffs, &sh_coeffs[i*max_coeffs*3], pos, cam_pos, rgb, training);
        ...
        int rMy=min(grid_y,max(0,(int)((pixel_y+(float)my_radius+(float)tile_h-1.0f)/(float)tile_h)));
        if ((rMx-rmx)*(rMy-rmy)==0) return;
        float rgb[3]; computeColorFromSH(sh_degree, max_coeffs, &sh_coeffs[i*max_coeffs*3], pos, cam_pos, rgb, training);
