...
Vulkan end-to-end training gap closure now includes the AAA-Gaussians MCMC densification path, final-step CUDA parity for CLI training, and render-config parity for saved-PLY comparisons. `VkTrainingConfig::cap_max > 0` selects MCMC relocate/add densification; `cap_max <= 0` preserves legacy clone/split/prune. `gs3d_vk_train` now skips Adam on the terminal iteration to match the CUDA harness final forward/backward/no-step contract and exposes `--proper_ewa 0|1` so eval_3D/proper-EWA experiments can match CUDA `ExtendedSettings`. Saved-PLY standalone 2D comparison must render with `--proper_ewa 0`, matching VulkanTrainer's current training default; otherwise the same PLY shows basketball distortion/white blocks from a preprocessing-mode mismatch.

## Test Counts (2026-04-29 current)
- Build: `cmake --build harmonyos_3dgs/build` OK after final-step no-update mode, `gs3d_vk_train --proper_ewa`, `gs3d_vk_render --proper_ewa`, MCMC densification, VulkanAdam resize, opacity reset, and eval_3D parity-mode training changes.
...
- Full CTest baseline after final-step no-update mode, train/render `--proper_ewa`, MCMC densification, and `filter_3D` propagation: **314/314 tests passed** (`ctest --test-dir harmonyos_3dgs/build --output-on-failure`, latest 40.63s; expected inventory is 66 .cpp files).
...
### S14 — 2026-04-29 (current) — Vulkan MCMC densification port
...
- Verification: build OK; targeted MCMC/Adam/densification/training/config/filter tests **54/54 PASS**; targeted final-step/trainer/densification parity tests **62/62 PASS**; parity-sensitive regression **42/42 PASS** with expected skips; full CTest **314/314 PASS** after train/render `--proper_ewa` CLI exposure. 5000-step basketball 2D MCMC with `cap_max=50000` and no opacity reset reached CUDA/VK final N **24680/24680**; with final-step skip and `--proper_ewa 0`, saved-PLY standalone render matches the VK training-final render at **74.68 dB**. 5000-step basketball eval_3D/proper_ewa MCMC with `cap_max=30000` reached CUDA/VK final N **24680/24680**, `CUDA vs GT 27.13 dB`, `VK saved vs GT 27.25 dB`, `VK saved vs CUDA 29.37 dB`, and `VK saved vs train 59.04 dB`. Independent subagent diff review found no blockers after the final-step/render-config pass; latest review noted the eval_3D comparison script is now tracked under `harmonyos_3dgs/tools/`.
