...
## Current Phase (merge-training-master — 2026-05-12)
Training/profiling sync into the master cascade/config line is integrated in isolated branch `worktree-merge-training-master`. Training-priority non-parity `eval_3D` replay-order support, the conservative profiling packed-tile-range Fuchsia path, O1 training timing/TileBinner reuse, and O2 RasterizerVulkan sync-path buffer reuse are now ported and validated while preserving master `SplattingSettings`, cascade trace/config validation, rasterize specialization IDs, replay-order bindings 14/15, and the non-trace HEAD_W=8 fallback. Current test inventory is **328 tests / 73 .cpp files**.

## Rasterizer Sync-Path Reuse Optimization (2026-05-12)
- Reused `RasterizerVulkan::rasterize()` Layer-1 sync-path buffers with grow-only cached SSBOs/UBOs for values, fallback tile ranges, means2D, packed conic/opacity, RGB, image/T/n_contrib outputs, eval_3D inputs, dummy eval_3D bindings, and replay-order sideband buffers. Layer-2 record-mode buffers and `rasterize_traced()` diagnostic allocations were intentionally left unchanged.
- Added fail-fast eval_3D input validation so cached eval_3D buffers cannot silently reuse stale data when `gauss2screen`, `opacities_2d`, `cov3D_inv`, or `mean_offset` are missing.
- Preserved the rasterizer ABI: replay-order bindings remain **14/15**, cascade trace remains **16..32**, rasterize specialization IDs remain `EVAL_3D=0`, `TRACE_ENABLED=1`, `SORT_MODE=2`, `EVAL3D_RAW_REPLAY=3`, Fuchsia sort remains opt-in, and no `rasterize.comp` behavior changed.
- Validation: `git diff --check` PASS for the rasterizer files; build PASS; focused rasterizer/forward **8/8 PASS**; trainer/MCMC/Adam/densification/VkVsPyReference **35/35 PASS**; Fuchsia/TileRange/Sorter **9/9 PASS**; `VkVsCudaFirstLoss` **12/12 PASS**; basketball/cascade-sensitive focus **9/9 PASS** with expected skips; full CTest **328/328 PASS** in **2831.92s** with the four expected fixture-dependent skips.
- O2 benchmark artifact: `build/compare_runs/radix_gt100k_perf_3step_500k_fuchsia_o2`. With `GS3D_USE_FUCHSIA_SORT=1 GS3D_VK_TRAIN_STAGE_TIMERS=1`, total pairs stayed above 100000 (`R≈3.23M`) and `VK saved vs CUDA` remained **25.72 dB**. The script exited nonzero because its full-GT gap gate reported **2.16 dB > 2.00 dB**, matching the same GT gap reported in O1; this is not a VK/CUDA PSNR regression.
- Stage timing comparison against the O1 baseline shows the intended rasterizer-host-churn improvement: rasterizer **1256.663/1877.950/1880.715 ms → 950.623/1023.436/1230.353 ms** for steps 0/1/2. Overall VK train CLI was **15.17s** in this O2 run versus **10.77s** in the saved O1 report, so total runtime is still dominated by broader per-run variance and non-rasterizer stages rather than solved by O2 alone.

## Profiling Packed Tile-Range Integration (2026-05-09)
- Added a dedicated packed-keyval tile-range shader and `TileRangeKeyLayout` selector so the Fuchsia sorted keyval buffer can generate tile ranges on GPU without changing the legacy `TileRangePass` contract. The default tile-range path still reads legacy keys with tile id in the high 32 bits.
- `SorterVulkan` now clears stale host/GPU outputs at sort entry, publishes `keyvals_sorted_gpu` and `tile_ranges_gpu` in the Fuchsia env-on path, and still downloads host mirrors plus reconstructs legacy `keys_sorted`/`values_sorted` for current rasterizer/backward compatibility.
- Unsafe profiling deltas were not ported: replay-order bindings remain **14/15**, cascade trace remains **16..32**, rasterize specialization IDs remain unchanged, and Fuchsia feature requirements remain opt-in rather than global Vulkan initialization requirements.
- Validation: shader build PASS; full build PASS; sorter/Fuchsia/tile-range focus **22/22 PASS**; forward/rasterizer/backward focus **20/20 PASS**; training/VkVsPyReference focus **35/35 PASS**; cascade/trace focus **4/4 PASS** with expected skips; basketball focus **7/7 PASS** with expected skips; `VkVsCudaFirstLoss` **12/12 PASS**; full CTest **328/328 PASS** in **2535.72s** with the four existing fixture-dependent skips.
- Cam0 render benchmark on `runs/full_cap500000_step30000_vk_cuda3373529_literal_match/vk_trained_cap500000_step30000.ply`: fallback `GS3D_USE_FUCHSIA_SORT=0` averaged **1548.5 ms render / 654.9 ms sorter / 2.46s wall** over 3 runs; Fuchsia env-on averaged **1121.6 ms render / 98.2 ms sorter / 1.88s wall**. Env-on output is deterministic but not bit-identical to fallback on this full scene (`RMSE 0.00883`, `PSNR 41.08 dB`, max abs `0.7987`), so the faster path remains opt-in and should not be treated as a default parity path yet.
