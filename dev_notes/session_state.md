...
- Full CTest after feature-matrix, render-SH, DSSIM optimization, and Gate_P2-P4 opening: **315/315 tests passed** (`ctest --test-dir harmonyos_3dgs/build --output-on-failure`, latest 42.94s; expected inventory is 66 .cpp files).
...
- First-loss intermediate gate validation: `VkVsCudaFirstLoss` **12/12 passed** with expected skips for P5-P7/L1; Gate_P2 validates eval_3D opacity-lane parity, Gate_P3 validates SH-evaluated RGB parity on overlap-active Gaussians, and Gate_P4 validates exact active-set radii parity with sentinel-scale radii masked and finite ceil-boundary off-by-one deltas bounded to `max(32, ceil(0.01% of finite radii))`.
...
| L1b | VK↔CUDA same-ply forward (basketball cam0) | **54.84 dB** (5.16 dB to 60 dB target). Gate_I1-I4 + P1-P4 PASS; P5-P7 SKIP |
...
| VK-CUDA L1b parity (PSNR≥60dB) | IN PROGRESS | S8-S16 | 25.3→54.84 dB. Phase 1 (Gate_I1-I4) + Gate_P1-P4 done; need Gate_P5-P7 implementation |
